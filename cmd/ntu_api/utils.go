...
func validateDate(startDate, targetDate time.Time) error {
	...
	if currentDate.Before(targetDate) {
		return &ErrDateRange{message: fmt.Sprintf("%s is in the future", targetDate.Format(TIMEFORMAT))}
	}

	if startDate.IsZero() {
		return &ErrNullDate{message: "start date is not initialized"}
	}

	if startDate.AddDate(0, 6, 0).Before(currentDate) {
		return &ErrDateRange{message: "start date is too outdated"}
	}

	if targetDate.Before(startDate) {
		return &ErrDateRange{message: "target date is before start date"}
	}
